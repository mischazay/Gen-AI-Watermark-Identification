...
\node[section] (embedding_section) at (0,0) {
    \begin{tikzpicture}[node distance=0.6cm]
        % avoid title overlap
        \node[font=\scriptsize\bfseries] at (0,0.5) {Embedding Pipeline};
...
% EXTRACTION PIPELINE 
\node[section, below=1cm of embedding_section] (extraction_section) {
    \begin{tikzpicture}[node distance=0.6cm]
        % avoid title overlap
        \node[font=\scriptsize\bfseries] at (0,0.5) {Extraction Pipeline};
        ...
        % Clearer vertical flow arrows
        \draw[arrow] (attacked_input) -- (vae_encoder);
        \draw[arrow] (vae_encoder) -- (latent_repr);
        \draw[arrow] (vae_encoder) -- (ddim_inversion);
        \draw[arrow] (ddim_inversion) -- (estimated_latent);
        \draw[arrow] (ddim_inversion) -- (inverse_sampling);
        \draw[arrow] (inverse_sampling) -- (decryption);
        \draw[arrow] (inverse_sampling) -- (majority_voting);
        \draw[arrow] (decryption) -- (extracted_watermark);
        \draw[arrow] (majority_voting) -- (extracted_watermark);
        \draw[arrow] (majority_voting) -- (ber_calculation);
